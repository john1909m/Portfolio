...
export const Skills = () => {
  ...
  return (
    <section ref={sectionRef} className={`skills-container py-10 transition-all duration-300 ${isDarkMode ? 'bg-blue-950 text-white' : 'bg-blue-50 text-gray-800'}`}>
      <div className="container mx-auto px-4 sm:px-6">
        ...
        <div className="category-nav flex flex-wrap justify-center mb-5 gap-2 sm:gap-4">
          {skillCategories.map(category => (
            <button
              ...
              className={`category-btn flex items-center px-4 py-2 rounded-full text-xs sm:text-sm transition-all duration-300 border ${
        ...
        <div className="skills-display">
          {skillCategories.map(category => (
            <div 
              key={category.id}
              className={`skills-category-container transition-all duration-500 transform ${
                activeCategory === category.id 
                  ? 'opacity-100 translate-x-0 relative' 
                  : 'absolute opacity-0 translate-x-8 pointer-events-none'
              }`}
              style={{ 
                visibility: activeCategory === category.id ? 'visible' : 'hidden',
                position: activeCategory === category.id ? 'relative' : 'absolute',
                top: activeCategory === category.id ? 'auto' : 0
              }}
            >
              <div 
                key={`grid-${category.id}-${isVisible}`} 
                className="skills-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 pb-6"
              >
